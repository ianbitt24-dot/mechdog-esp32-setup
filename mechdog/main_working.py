  ...
  while True:
    if ble.is_connected():
      if ble.contains_data("CMD"):
        ...
        print("Received command:", _COMMAND, "Data:", _REC_PARSE_VALUE)
        if (_COMMAND==6):
        ...
        if (_COMMAND==7):
          ...
          # Don't continue, let it fall through to process
...
def start_main1():
  ...
  while True:
    ...
    if time.ticks_ms() > arm_tick:
      if arm_step == 0:
        print("Checking _ARM_ACTION:", _ARM_ACTION, "arm_step:", arm_step)
        if _ARM_ACTION == 3:  # servo 9 control
          print("Action 3 - Servo 9 control, data:", _REC_PARSE_VALUE)
          ...
          _ARM_ACTION = 0
        elif _ARM_ACTION == 4:  # servo 10 control
          if len(_REC_PARSE_VALUE) > 2:
            direction = int(_REC_PARSE_VALUE[2])
            if direction == 1:
